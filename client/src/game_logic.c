...
// 화면에 이미 떠 있는 단어인지 확인
static bool is_word_active(const char *word) {
    for (int i = 0; i < MAX_WORDS; ++i) {
        if (words[i].active && strcmp(words[i].text, word) == 0)
            return true;
    }
    return false;
}
...
static void spawn_word(void)
{
    ...
    for (int i = 0; i < MAX_WORDS; ++i) {
        if (words[i].thread_id == 0 || words[i].to_remove) {
            ...
            /* ─── 단어 정보 초기화 ─── */
            /* 같은 단어가 이미 떠 있으면 다시 뽑기 */
            const char* pick;
            int tries = 0;
            do {
                pick = dynamic_word_list[rand() % dynamic_word_count];
                tries++;
                if (tries > dynamic_word_count) break;
            } while (is_word_active(pick));
...
static void draw_game_screen(void) {
  ...
  if (over) {
    ...
    mvprintw(GAME_AREA_START_Y+GAME_AREA_HEIGHT/2+1,
             GAME_AREA_START_X+(GAME_AREA_WIDTH-strlen("Press any key to continue..."))/2,
  ...
  if (ch == '\n' || ch == ' ') {
    if (input_pos > 0) {
      ...
      for (int i = 0; i < MAX_WORDS; ++i) {
        ...
        int prio = (words[i].wtype == WORD_KILL)  ? 0 :
      ...
      if (target_idx != -1) {
        ...
        pthread_mutex_lock(&game_state_mutex);
        if (t == WORD_KILL) game_is_over = true;
        else {
...
int run_rain_typing_game(const char *user_id) {
  ...
  if (GAME_AREA_HEIGHT < 5 || GAME_AREA_WIDTH < 20) {
    clear();
    mvprintw(screen_height_cache / 2,
             (screen_width_cache - strlen("Screen too small.")) / 2,
             "Screen too small.");
    refresh();
  ...
  if (pthread_mutex_init(&screen_mutex, NULL) != 0 || pthread_mutex_init(&words_mutex, NULL) != 0 ||
    ...
    clear();
    mvprintw(screen_height_cache / 2,
             (screen_width_cache - strlen("Mutex init failed.")) / 2,
             "Mutex init failed.");
    refresh();
    nodelay(stdscr, FALSE);
    getch();
